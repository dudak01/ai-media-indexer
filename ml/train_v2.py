"""
=============================================================================
ФАЙЛ МОДЕЛИ — Обучение (fine-tuning) DistilBERT NER (версия v2)
...
Описание:
    Версия v2 файла обучения NER. Отличия от train.py (v1):
        ...
        - результаты сохраняются в model2.pt и metrics_v2.json
...
METRICS_PATH = BASE_DIR / "ml" / "metrics_v2.json"
MODEL_SAVE_PATH = WEIGHTS_DIR / "model2.pt"
...
def plot_training_history(history, save_path):
    ...
    ax1.set_ylabel("Loss")
    ax1.set_title("Динамика функции потерь (v2)")
    ax1.legend()
    ...
    ax2.set_title("Динамика точности (v2)")
    ax2.legend()
...
def train_model():
    ...
    output = {
        "model_version": "v2_multilingual_cased",
    ...
    plot_training_history(history, PLOTS_DIR / "training_history_v2.png")
    plot_confusion_matrix(
        np.array(final_metrics["confusion_matrix"]),
        LABELS,
        PLOTS_DIR / "confusion_matrix_v2.png",
    )

    print(f"\n[OK] Обучение v2 завершено")
